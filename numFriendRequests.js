...
function numFriendRequests(ages) {
    ...
    // For each possible age x (sender)
    for (let senderAge = 1; senderAge <= 120; senderAge++) {
        if (ageFrequency[senderAge] === 0) continue;
        
        // Calculate minimum valid age for receiver
        const minReceiverAge = Math.floor(0.5 * senderAge + 7) + 1;
        
        // For each possible age y (receiver)
        for (let receiverAge = minReceiverAge; receiverAge <= senderAge; receiverAge++) {
            if (ageFrequency[receiverAge] === 0) continue;
            
            // Check the third condition: age[y] > 100 && age[x] < 100
            if (receiverAge > 100 && senderAge < 100) continue;
            
            // Calculate number of requests
            if (senderAge === receiverAge) {
                // Same age: each person sends to all others of same age
                totalRequests += ageFrequency[senderAge] * (ageFrequency[senderAge] - 1);
            } else {
